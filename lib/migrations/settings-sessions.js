 ...
 * @param options - settings 服务, 可选的旧 namespace scope, 状态服务与 logger.
...
export async function migrateSettingsSessions({ settings, scope, service, namespace, logger }) {
  ...
  if (storedVersion >= SETTINGS_SCHEMA_VERSION) {
    ...
    if (scope === undefined) return { imported: 0, rejected: 0, cleaned: false }
  ...
  const cleanedSection = withoutLegacyField(userSection)
  if (cleanedSection !== null && scope !== undefined) await scope.replace(cleanedSection)
  ...
  logger.info(
    ...
    + `${cleanedSection === null || scope === undefined ? '' : ', 已从 settings 移除 sessions 字段'}`,
  )
  return { imported, rejected, cleaned: cleanedSection !== null && scope !== undefined }
}
