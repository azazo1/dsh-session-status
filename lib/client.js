/**
 * dsh-session-status — 浏览器 bundle（自包含，无跨模块 import）。
 *
 * 双轨制：本文件 `exports.inject` 写 Cordis 服务名（slots / settingsScope /
 * sessions），不是 NPM 包名；package.json 的 dsh.client.inject 才写包名。
 *
 * 数据来自两条通道：
 *  1. settings scope（namespace dsh-session-status）—— 只剩标签定义 labels 与
 *     内置三态颜色覆盖 overrides，仍是响应式读写。
 *  2. host HTTP 路由（/plugins/dsh-session-status/...）—— 会话 -> 标签 key 的
 *     映射存在 host 的 storage domain 里，浏览器只能通过 fetch 读写：本地保留
 *     一份快照做乐观更新，写后回填服务端快照，另有低频轮询 + 窗口聚焦时刷新，
 *     以覆盖其它标签页/浏览器造成的变更。
 *
 * 三块 UI：
 *  1. 头部状态 pill + 选择器 —— 注入 `conversation.session.header.utilities`
 *     （scope: session，props.sessionId 可用），单击主按钮循环四态, 右侧箭头弹出
 *     内置三态 + 自定义 + 清除。
 *  2. 会话列表行状态点: 从行组件的 React key 与 node/result.id 确认 session id,
 *     支持同名会话和搜索结果; MutationObserver/RAF 节流, data-owner 自清理.
 *     单击状态点在三个内置态之间循环, 右键清除; 原生 click / contextmenu 里
 *     阻断冒泡以免打开会话.
 *  3. 会话 hover 卡状态行: 沿 portal 的 React 归属找到同一个 session id,
 *     把标签追加进卡内容列, 与原生运行状态并列显示.
 *  4. 插件页配置卡片 —— 注入 `plugins.bundle.config` (key = 包名),
 *     内置三态名称固定, 颜色可调 (overrides, 点默认色即恢复);
 *     自定义标签可增删改 (名称 + 8 色板).
 *
 * 样式集中在注入的一份 <style> (ensureStyleSheet), 悬停/聚焦/过渡全走 CSS 伪类,
 * DOM 侧只写 CSS 变量, 避免自触发 MutationObserver; 动态色由标签色现算 rgba.
 *
 * 纯逻辑（resolveLabels / assignStatus ...）与
 * lib/status-store.js 同源 —— 本文件是浏览器端内联副本，通过 `__statusUtils`
 * 导出供 client-shape.test.mjs 与 status-store.test.mjs 交叉断言，防止漂移。
 */
window.__ModuleLoader__.load({
  id: 'dsh-session-status',
  factory: (require) => {
    var module = { exports: {} }
    var exports = module.exports
    Object.defineProperty(exports, Symbol.toStringTag, { value: 'Module' })
    var react = require('react')
    // 宿主同一套原生 UI 原子: 下拉菜单, 提示气泡, 开关, 输入框, 按钮, 图标.
    var primitives = require('@deepseek-ai/dsh-client-ui-primitives')
    var Button = primitives.Button
    var IconChevronDownOutlineRegular = primitives.IconChevronDownOutlineRegular
    var Input = primitives.Input
    var Menu = primitives.Menu
    var Switch = primitives.Switch
    var Tooltip = primitives.Tooltip

    // ---------------------------------------------------------------- 纯逻辑

    var BUILTIN_LABELS = [
      { key: 'active', name: '进行中', color: '#22c55e', builtin: true },
      { key: 'done', name: '已结项', color: '#3b82f6', builtin: true },
      { key: 'paused', name: '搁置中', color: '#9ca3af', builtin: true },
    ]
    var PALETTE = [
      '#ef4444', '#f59e0b', '#8b5cf6', '#14b8a6', '#ec4899',
      '#f97316', '#06b6d4', '#a3e635',
    ]

    /** 把用户输入规范化为 #rrggbb；非法输入返回 null（支持 #RGB 简写）。 */
    function normalizeHex(value) {
      if (typeof value !== 'string') return null
      var v = value.trim()
      var long = /^#([0-9a-fA-F]{6})$/.exec(v)
      if (long) return '#' + long[1].toLowerCase()
      var short = /^#([0-9a-fA-F]{3})$/.exec(v)
      if (short) {
        var hex = short[1].toLowerCase()
        return '#' + hex[0] + hex[0] + hex[1] + hex[1] + hex[2] + hex[2]
      }
      return null
    }
    /**
     * 把标签色换算成 rgba(r, g, b, alpha); 非法颜色返回 null, alpha 越界夹到 [0, 1].
     * 淡染底色与描边用它现算, 不依赖 color-mix().
     */
    function rgbaFromHex(value, alpha) {
      var hex = normalizeHex(value)
      if (hex === null) return null
      var r = parseInt(hex.slice(1, 3), 16)
      var g = parseInt(hex.slice(3, 5), 16)
      var b = parseInt(hex.slice(5, 7), 16)
      var raw = typeof alpha === 'number' && isFinite(alpha) ? alpha : 1
      var a = Math.min(1, Math.max(0, raw))
      return 'rgba(' + r + ', ' + g + ', ' + b + ', ' + a + ')'
    }

    var BUILTIN_KEYS = {}
    for (var bi = 0; bi < BUILTIN_LABELS.length; bi++) BUILTIN_KEYS[BUILTIN_LABELS[bi].key] = true

    function builtinDefaultColor(key) {
      for (var i = 0; i < BUILTIN_LABELS.length; i++) {
        if (BUILTIN_LABELS[i].key === key) return BUILTIN_LABELS[i].color
      }
      return undefined
    }

    /** 把内置标签的覆盖补丁应用到基线上（当前只写 color，name 预留）。 */
    function applyBuiltinOverride(label, override) {
      if (override === null || typeof override !== 'object') return label
      var out = null
      var KEYS = ['color', 'name']
      for (var i = 0; i < KEYS.length; i++) {
        var key = KEYS[i]
        if (typeof override[key] !== 'string' || override[key] === '') continue
        if (!out) out = { key: label.key, name: label.name, color: label.color, builtin: true }
        out[key] = override[key]
      }
      return out || label
    }

    function resolveLabels(value) {
      var raw = Array.isArray(value)
        ? value
        : (value !== null && typeof value === 'object' && Array.isArray(value.labels) ? value.labels : [])
      var overrides = (!Array.isArray(value) && value !== null && typeof value === 'object'
        && value.overrides !== null && typeof value.overrides === 'object')
        ? value.overrides
        : {}
      var seen = {}
      var merged = []
      for (var i = 0; i < BUILTIN_LABELS.length; i++) {
        merged.push(applyBuiltinOverride(BUILTIN_LABELS[i], overrides[BUILTIN_LABELS[i].key]))
        seen[BUILTIN_LABELS[i].key] = true
      }
      for (var j = 0; j < raw.length; j++) {
        var label = raw[j]
        if (label === null || typeof label !== 'object') continue
        if (typeof label.key !== 'string' || seen[label.key]) continue
        if (typeof label.name !== 'string' || label.name === '') continue
        if (typeof label.color !== 'string' || label.color === '') continue
        seen[label.key] = true
        merged.push({
          key: label.key, name: label.name, color: label.color, builtin: false,
          icon: typeof label.icon === 'string' ? label.icon : undefined,
        })
      }
      return merged
    }

    function findLabel(labels, key) {
      if (!Array.isArray(labels)) return undefined
      for (var i = 0; i < labels.length; i++) {
        if (labels[i].key === key) return labels[i]
      }
      return undefined
    }

    function labelIconKey(key) {
      switch (key) {
        case 'active': return 'bolt'
        case 'done': return 'check'
        case 'paused': return 'pause'
        default: return 'tag'
      }
    }

    var LABEL_ICONS = [
      'tag', 'flag', 'star', 'fire', 'clock', 'bookmark', 'rocket',
      'target', 'pin', 'bell', 'bolt', 'check', 'pause',
    ]

    function resolveLabelIcon(label) {
      if (label === null || typeof label !== 'object' || typeof label.key !== 'string') return 'tag'
      if (label.key === 'active' || label.key === 'done' || label.key === 'paused') {
        return labelIconKey(label.key)
      }
      return LABEL_ICONS.indexOf(label.icon) !== -1 ? label.icon : 'tag'
    }

    function nextSessionStatus(currentKey) {
      var ORDER = ['active', 'done', 'paused']
      if (typeof currentKey !== 'string' || currentKey === '') return 'active'
      var index = ORDER.indexOf(currentKey)
      if (index === -1) return 'active'
      return index + 1 < ORDER.length ? ORDER[index + 1] : null
    }

    function assignStatus(sessions, sessionId, key) {
      var next = {}
      var src = sessions || {}
      for (var k in src) if (Object.prototype.hasOwnProperty.call(src, k)) next[k] = src[k]
      if (key === null || key === undefined || key === '') {
        delete next[sessionId]
      } else {
        next[sessionId] = key
      }
      return next
    }

    /**
     * 「新会话默认进行中」的判定：挑出需要补写 active 的会话 (与
     * lib/status-store.js 同源)。语义见那边的注释: 只认第一次亲眼出现的会话,
     * 取基线那一轮不写, 空白占位与快照未到位都留到下一轮。
     */
    function planAutoActive(input) {
      var seen = {}
      var order = []
      var src = input.seenIds || []
      for (var s = 0; s < src.length; s++) {
        if (seen[src[s]]) continue
        seen[src[s]] = true
        order.push(src[s])
      }
      if (input.enabled !== true) return { armed: false, seenIds: [], targets: [] }
      var ids = Array.isArray(input.ids) ? input.ids : []
      var summaries = input.summaries || {}
      var statuses = input.statuses || {}
      var arming = input.armed !== true
      var targets = []
      for (var i = 0; i < ids.length; i++) {
        var id = ids[i]
        if (!arming && seen[id]) continue
        var summary = summaries[id]
        if (!summary || summary.blank) continue
        if (arming) {
          if (seen[id]) continue
          seen[id] = true
          order.push(id)
          continue
        }
        if (input.statusesReady !== true) continue
        seen[id] = true
        order.push(id)
        if (statuses[id]) continue
        targets.push(id)
      }
      return { armed: true, seenIds: order, targets: targets }
    }

    function setOverride(overrides, key, patch) {
      var next = {}
      var src = overrides || {}
      for (var k in src) if (Object.prototype.hasOwnProperty.call(src, k)) next[k] = src[k]
      next[key] = Object.assign({}, next[key] || {}, patch)
      return next
    }

    function clearOverride(overrides, key) {
      if (overrides === null || overrides === undefined) return undefined
      var next = {}
      for (var k in overrides) {
        if (!Object.prototype.hasOwnProperty.call(overrides, k)) continue
        if (k === key) continue
        next[k] = overrides[k]
      }
      return Object.keys(next).length === 0 ? undefined : next
    }

    function generateLabelKey(name, existingKeys) {
      var base = String(name)
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '')
      var safe = /^[a-z]/.test(base) ? base : 'label-' + (base || 'custom')
      var key = safe
      var index = 2
      while (existingKeys.has(key)) {
        key = safe + '-' + index
        index += 1
      }
      return key
    }

    function nextPaletteColor(labels) {
      var used = {}
      var list = labels || []
      for (var i = 0; i < list.length; i++) used[list[i].color] = true
      for (var j = 0; j < PALETTE.length; j++) {
        if (!used[PALETTE[j]]) return PALETTE[j]
      }
      return PALETTE[0]
    }

    var __statusUtils = {
      resolveLabels, findLabel, labelIconKey, resolveLabelIcon, LABEL_ICONS,
      nextSessionStatus, assignStatus, generateLabelKey, nextPaletteColor,
      builtinDefaultColor, setOverride, clearOverride, normalizeHex, rgbaFromHex,
      planAutoActive,
    }

    // ------------------------------------------------------------ 样式表

    var STYLE_ELEMENT_ID = 'dsh-session-status-style'

    /**
     * 插件样式集中在注入的一份 <style> 里: 悬停, 聚焦, 过渡与主题色全走 CSS,
     * DOM 侧只写 CSS 变量, 这样点击与悬停不会改内联样式, MutationObserver 不会自激.
     * 该元素不带 data-owner, 否则会被 renderAll 的清理当作插件节点删掉.
     */
    var STYLE_CSS = [
      // 头部 pill: 形状, 尺寸与描边对齐右侧「在本地打开」的 split 按钮.
      // Menu 会在锚点外面再包一层 span, 所以那一层也要跟「在本地打开」的 menuAnchor 一样.
      '.dss-pill-anchor{display:inline-flex;flex:none;align-self:center;}',
      '.dss-pill{position:relative;display:inline-flex;flex:none;align-self:center;box-sizing:border-box;height:24px;font-family:var(--dsw-font-family,inherit);}',
      '.dss-pill-main,.dss-pill-caret{display:inline-flex;align-items:center;justify-content:center;box-sizing:border-box;border:.5px solid var(--dsw-alias-border-l4,rgba(128,128,128,.25));background:none;color:var(--dsw-alias-label-primary,#ddd);font-family:inherit;font-size:11px;line-height:16px;white-space:nowrap;cursor:pointer;transition:background-color var(--ds-transition-duration-fast,.1s) var(--ds-ease-in-out,ease);}',
      '.dss-pill-main{gap:4px;padding:3px 5px;border-right:0;border-radius:var(--dsw-radius-sm,8px) 0 0 var(--dsw-radius-sm,8px);background:var(--dss-tint,transparent);}',
      '.dss-pill-caret{padding:3px 4px 3px 3px;border-radius:0 var(--dsw-radius-sm,8px) var(--dsw-radius-sm,8px) 0;color:var(--dsw-alias-label-secondary,#aaa);}',
      '.dss-pill-main:hover,.dss-pill-main:focus-visible{background:var(--dss-tint-strong,var(--dsw-alias-interactive-bg-hover,rgba(128,128,128,.12)));outline:none;}',
      '.dss-pill-caret:hover,.dss-pill-caret:focus-visible{background:var(--dsw-alias-interactive-bg-hover,rgba(128,128,128,.12));outline:none;}',
      '.dss-pill-main:active,.dss-pill-caret:active{background:var(--dsw-alias-interactive-bg-active,rgba(128,128,128,.2));}',
      '.dss-pill[data-unset="true"] .dss-pill-main{color:var(--dsw-alias-label-secondary,#aaa);}',
      // 会话行状态点: 16x20 命中区与原生 16px 槽同宽, 不改变行首总宽度.
      '.dss-row-dot{display:inline-flex;align-items:center;justify-content:center;flex:none;box-sizing:border-box;width:16px;height:20px;border-radius:var(--dsw-radius-xs,4px);cursor:pointer;transition:background-color var(--ds-transition-duration-fast,.1s) var(--ds-ease-in-out,ease);}',
      '.dss-row-dot:hover,.dss-row-dot:focus-visible{background:var(--dsw-alias-interactive-bg-hover,rgba(128,128,128,.12));outline:none;}',
      '.dss-row-dot:active{background:var(--dsw-alias-interactive-bg-active,rgba(128,128,128,.2));}',
      // 设置卡片里少数原生原子没有的语义色 (删除按钮) 与布局类 (输入框占满剩余宽度).
      '.dss-danger{color:var(--dsw-alias-state-error-primary,#f87171);}',
      '.dss-grow{flex:1;min-width:0;}',
    ].join('')

    /** 幂等注入插件样式表 (在 document.head, 不参与 body 的 DOM 观察). */
    function ensureStyleSheet() {
      if (document.getElementById(STYLE_ELEMENT_ID)) return
      var style = document.createElement('style')
      style.id = STYLE_ELEMENT_ID
      style.textContent = STYLE_CSS
      ;(document.head || document.documentElement).appendChild(style)
    }

    /** 卸载时移除样式表. */
    function removeStyleSheet() {
      var style = document.getElementById(STYLE_ELEMENT_ID)
      if (style) style.remove()
    }

    // ------------------------------------------------------------ 共享状态

    var scopeRef = { scope: null }   // settingsScope.bind 的结果 (labels / overrides)
    var sessionsRef = { list: null } // ctx.sessions.list（ObservableSnapshot）

    /** host 路由路径；必须与 lib/http/routes.js 的常量一致。 */
    var STATUS_API = {
      statuses: '/plugins/dsh-session-status/statuses',
      status: '/plugins/dsh-session-status/status',
      prune: '/plugins/dsh-session-status/prune',
    }

    /** states 轮询周期：状态变更都是人工点按，低频足够。 */
    var STATUS_POLL_MS = 15000

    /**
     * 会话状态快照 (sessionId -> labelKey)。
     * 权威数据在 host 的 storage domain 里，这里是本地镜像：写操作先乐观更新，
     * 再用服务端返回的快照回填。
     */
    var statusRef = { map: {}, ready: false, listeners: [], inFlight: null }

    /**
     * 「新会话默认进行中」的本地进度: armed 表示已取过基线 (当前已有会话不动),
     * seen 是本次页面里已经判过的会话 id。页面重载即重置, 所以重载后第一轮
     * 只会重新取基线, 不会批量补写。
     */
    var autoActiveRef = { armed: false, seen: [] }

    /** 订阅状态快照变化（返回反订阅函数）。 */
    function subscribeStatus(callback) {
      statusRef.listeners.push(callback)
      return function () {
        var index = statusRef.listeners.indexOf(callback)
        if (index !== -1) statusRef.listeners.splice(index, 1)
      }
    }

    /** 通知状态快照订阅者（单个回调抛错不影响其它回调）。 */
    function notifyStatus() {
      var listeners = statusRef.listeners.slice()
      for (var i = 0; i < listeners.length; i++) {
        try {
          listeners[i]()
        } catch (error) {
          console.warn('[dsh-session-status] 状态订阅回调出错', error)
        }
      }
    }

    /** 用一份新快照替换本地镜像（相同内容不重复通知）。 */
    function applyStatuses(map) {
      var next = map && typeof map === 'object' ? map : {}
      if (JSON.stringify(next) === JSON.stringify(statusRef.map)) {
        statusRef.ready = true
        return
      }
      statusRef.map = next
      statusRef.ready = true
      notifyStatus()
    }

    /** 读取本地状态快照（渲染只读这里）。 */
    function statusMap() {
      return statusRef.map
    }

    /** 解析一次 JSON 响应；非 2xx 抛错。 */
    function readJsonResponse(response) {
      if (!response.ok) throw new Error('HTTP ' + response.status)
      return response.json()
    }

    /** 从 host 拉取全量状态；并发调用共用同一个请求。 */
    function refreshStatuses() {
      if (statusRef.inFlight) return statusRef.inFlight
      var pending = fetch(STATUS_API.statuses, { headers: { accept: 'application/json' } })
        .then(readJsonResponse)
        .then(function (body) { applyStatuses(body && body.statuses) })
        .catch(function (error) {
          console.warn('[dsh-session-status] 读取会话状态失败', error)
        })
        .then(function () { statusRef.inFlight = null })
      statusRef.inFlight = pending
      return pending
    }

    /** POST 一个小 JSON 并解析响应。 */
    function postJson(path, payload) {
      return fetch(path, {
        method: 'POST',
        headers: { 'content-type': 'application/json', accept: 'application/json' },
        body: JSON.stringify(payload),
      }).then(readJsonResponse)
    }

    /** 惰性清理：把当前会话列表交给 host，删掉已不存在会话的记录。 */
    function pruneStatuses() {
      var live = liveSessionIds()
      if (!live || live.size === 0) return Promise.resolve()
      return postJson(STATUS_API.prune, { liveIds: Array.from(live) })
        .then(function (body) { applyStatuses(body && body.statuses) })
        .catch(function (error) { console.warn('[dsh-session-status] 清理会话状态失败', error) })
    }

    /**
     * 把 sessionId 的状态设为 key（null 清除）：先乐观更新本地快照，再写 host，
     * 失败则回读一次以恢复服务端真相。
     */
    function setSessionStatus(sessionId, key) {
      if (!sessionId) return
      applyStatuses(assignStatus(statusMap(), sessionId, key))
      postJson(STATUS_API.status, { sessionId: sessionId, labelKey: key === undefined ? null : key })
        .then(function (body) { applyStatuses(body && body.statuses) })
        .catch(function (error) {
          console.warn('[dsh-session-status] 写入会话状态失败', error)
          return refreshStatuses()
        })
        .then(function () { return pruneStatuses() })
    }

    // ------------------------------------------------------------ 工具函数

    function ownNodeSelector() {
      return '[data-owner="dsh-session-status"]'
    }

    function removeOwnNodes() {
      var nodes = document.querySelectorAll(ownNodeSelector())
      for (var i = 0; i < nodes.length; i++) nodes[i].remove()
    }

    /** 当前会话 id 集合；列表未就绪（phase !== 'ready'）时返回 null（不清理）。 */
    function liveSessionIds() {
      var list = sessionsRef.list ? sessionsRef.list.getSnapshot() : null
      if (!list || list.phase !== 'ready') return null
      return new Set(list.ids || [])
    }

    /**
     * 提交一次 settings 写入并盯住结果: 宿主拒绝时在控制台留线索 (最常见的原因是
     * 宿主进程还没重启, 新增的配置字段没注册进 schema); 界面反馈由调用方负责.
     * @returns 原写入 promise (调用方可继续链式处理), 非 promise 时返回 undefined.
     */
    function applyScopeWrite(pending, field) {
      if (!pending || typeof pending.then !== 'function') return undefined
      pending.then(function (ok) {
        if (ok === false) {
          console.warn('[dsh-session-status] settings 写入被宿主拒绝: ' + field
            + ' (刚改过插件配置字段的话, 需要重启 dsh web 让宿主重新注册 schema)')
        }
      }, function (error) {
        console.warn('[dsh-session-status] settings 写入失败: ' + field, error)
      })
      return pending
    }

    /** 写入自定义标签列表（不含内置；空则 unset，保持文档干净）。 */
    function writeLabels(nextLabels) {
      if (!scopeRef.scope) return undefined
      var merged = resolveLabels(nextLabels)
      var customs = merged.filter(function (label) { return !label.builtin })
      if (customs.length === 0) {
        return applyScopeWrite(scopeRef.scope.unset('labels'), 'labels')
      }
      return applyScopeWrite(scopeRef.scope.set('labels', customs), 'labels')
    }

    /** 写入内置标签颜色覆盖；color 等于默认色时清除该覆盖（空映射 unset 不落盘）。 */
    function writeBuiltinOverride(key, color) {
      if (!scopeRef.scope || !key) return undefined
      var snap = scopeRef.scope.getSnapshot()
      var value = snap.status === 'ready' && snap.value ? snap.value : null
      var overrides = (value && value.overrides) || {}
      if (color === builtinDefaultColor(key) || !color) {
        var next = clearOverride(overrides, key)
        if (next) return applyScopeWrite(scopeRef.scope.set('overrides', next), 'overrides')
        return applyScopeWrite(scopeRef.scope.unset('overrides'), 'overrides')
      }
      return applyScopeWrite(scopeRef.scope.set('overrides', setOverride(overrides, key, { color: color })), 'overrides')
    }

    /** 写入「新会话默认进行中」开关；关闭即 unset（默认值就是关闭, 不落盘）。 */
    function writeDefaultActive(enabled) {
      if (!scopeRef.scope) return undefined
      if (enabled) return applyScopeWrite(scopeRef.scope.set('defaultActive', true), 'defaultActive')
      return applyScopeWrite(scopeRef.scope.unset('defaultActive'), 'defaultActive')
    }

    // ------------------------------------------------- 会话行与悬浮卡片标记

    /**
     * DSH 的会话行组件以 session id 为 React key, 并通过 node/result props 接收同一 id.
     * DOM 没有公开的会话 id, 因此只读 React 挂载信息并交叉校验, 不用标题推断身份.
     * portal 的 return 链也回到所属会话行. 宿主结构不匹配时不注入.
     */
    function sessionIdForElement(element, list) {
      var keys = Object.keys(element)
      var fiber = null
      for (var i = 0; i < keys.length; i++) {
        if (keys[i].indexOf('__reactFiber$') === 0) {
          fiber = element[keys[i]]
          break
        }
      }
      while (fiber) {
        var id = fiber.key
        var props = fiber.memoizedProps
        if (typeof id === 'string' && props && (
          (props.node && props.node.id === id) || (props.result && props.result.id === id)
        )) {
          var summary = list.byId && list.byId[id]
          return summary && !summary.blank ? id : null
        }
        fiber = fiber.return
      }
      return null
    }

    /** 普通行或搜索结果标题位于内容容器的直属文本 span, 跳过原生图标和插件节点. */
    function rowTitleElement(row) {
      var container = row.tagName === 'BUTTON' ? row.firstElementChild : row
      if (!container) return null
      for (var i = 0; i < container.children.length; i++) {
        var child = container.children[i]
        if (child.tagName === 'SPAN' && child.children.length === 0
          && !child.hasAttribute('data-owner') && child.textContent.trim() !== '') return child
      }
      return null
    }

    function collectStatusContext() {
      if (!scopeRef.scope) return null
      var snap = scopeRef.scope.getSnapshot()
      if (snap.status !== 'ready' || !snap.value) return null
      var list = sessionsRef.list ? sessionsRef.list.getSnapshot() : null
      if (!list || list.phase !== 'ready') return null
      return {
        list: list,
        labels: resolveLabels(snap.value),
        sessionsMap: statusMap(),
        autoActive: snap.value.defaultActive === true,
      }
    }

    function labelForSession(ctx, id) {
      return id ? findLabel(ctx.labels, ctx.sessionsMap[id]) : undefined
    }

    /** 只在图标变化时修改 DOM, 避免 MutationObserver 被自身更新持续触发. */
    function updateIcon(element, label) {
      var markup = iconSvgMarkup(resolveLabelIcon(label), label.color, 12)
      if (element.getAttribute('data-icon-markup') === markup) return
      element.innerHTML = markup
      element.setAttribute('data-icon-markup', markup)
    }

    /** 头部 pill 主按钮的提示文字 (四态循环, 含清除). */
    function cycleHint(name) {
      return name + ' (单击循环: 进行中 → 已结项 → 搁置中 → 无)'
    }

    /** 会话列表行状态点的提示文字 (三态循环, 清除走右键). */
    function rowDotHint(name) {
      return name + ' (单击循环: 进行中 → 已结项 → 搁置中; 右键清除)'
    }

    /**
     * 行状态点的三态循环: 进行中 -> 已结项 -> 搁置中 -> 进行中.
     * 复用四态循环, 只在它给出「无」时回卷到「进行中」, 这样列表里的点不会自己消失.
     */
    function nextRowDotStatus(currentKey) {
      return nextSessionStatus(currentKey) || 'active'
    }

    /**
     * 行状态点的单击/回车: 循环三态. 阻断冒泡, 否则会触发会话行的 onClick 打开会话.
     * 会话 id 每次从标记上现读, 因为标记会随行复用.
     */
    function activateRowDot(event) {
      event.preventDefault()
      event.stopPropagation()
      var id = event.currentTarget.getAttribute('data-session-id')
      if (id) setSessionStatus(id, nextRowDotStatus(statusMap()[id]))
    }

    /** 行状态点的右键: 清除状态 (并压掉浏览器右键菜单). */
    function clearRowDot(event) {
      event.preventDefault()
      event.stopPropagation()
      var id = event.currentTarget.getAttribute('data-session-id')
      if (id) setSessionStatus(id, null)
    }

    function onRowDotKeyDown(event) {
      if (event.key !== 'Enter' && event.key !== ' ') return
      activateRowDot(event)
    }

    function renderRowDots(ctx, kept) {
      var rows = document.querySelectorAll('[role="treeitem"][aria-selected]')
      for (var r = 0; r < rows.length; r++) {
        var row = rows[r]
        var id = sessionIdForElement(row, ctx.list)
        var label = labelForSession(ctx, id)
        if (!label) continue
        var title = rowTitleElement(row)
        if (!title) continue
        var container = title.parentElement
        var mark = container.querySelector(':scope > [data-owner="dsh-session-status"][data-owner-part="row"]')
        if (!mark) {
          mark = document.createElement('span')
          mark.setAttribute('data-owner', 'dsh-session-status')
          mark.setAttribute('data-owner-part', 'row')
          mark.className = 'dss-row-dot'
          mark.setAttribute('role', 'button')
          mark.setAttribute('tabindex', '0')
          // 状态点自身不是拖拽源, 行拖拽排序仍由会话行决定.
          mark.setAttribute('draggable', 'false')
          mark.addEventListener('click', activateRowDot)
          mark.addEventListener('contextmenu', clearRowDot)
          mark.addEventListener('keydown', onRowDotKeyDown)
          container.insertBefore(mark, container.firstChild)
        }
        mark.setAttribute('data-session-id', id)
        mark.setAttribute('data-status-key', label.key)
        var hint = rowDotHint(label.name)
        if (mark.title !== hint) mark.title = hint
        var aria = '会话状态: ' + label.name + ', 单击循环切换, 右键清除'
        if (mark.getAttribute('aria-label') !== aria) mark.setAttribute('aria-label', aria)
        updateIcon(mark, label)
        kept.add(mark)
      }
    }

    function installRowDots() {
      var observers = []
      var rafId = 0
      var onReady = null
      function schedule() {
        if (rafId) return
        rafId = requestAnimationFrame(function () {
          rafId = 0
          renderAll()
        })
      }
      function startObserving() {
        if (!document.body) return
        var observer = new MutationObserver(schedule)
        observer.observe(document.body, { childList: true, subtree: true, characterData: true })
        observers.push(observer)
        schedule()
      }
      if (document.readyState === 'loading') {
        onReady = startObserving
        document.addEventListener('DOMContentLoaded', onReady)
      } else {
        startObserving()
      }
      return function cleanup() {
        if (onReady) document.removeEventListener('DOMContentLoaded', onReady)
        for (var i = 0; i < observers.length; i++) observers[i].disconnect()
        if (rafId) cancelAnimationFrame(rafId)
        removeOwnNodes()
      }
    }

    /** 悬浮卡片经 React portal 仍归属原会话行, 不需要标题反查. */
    function renderHoverStatus(ctx, kept) {
      var cards = document.querySelectorAll('div[role="button"]')
      for (var c = 0; c < cards.length; c++) {
        var card = cards[c]
        var style = card.getAttribute('style') || ''
        if (style.indexOf('left:') === -1 || style.indexOf('top:') === -1) continue
        var id = sessionIdForElement(card, ctx.list)
        var label = labelForSession(ctx, id)
        if (!label) continue
        // HoverCard 复制反馈用 span 替换内容列, 此时不显示状态行.
        var container = card.firstElementChild
        if (!container || container.tagName !== 'DIV') continue
        var line = container.querySelector(':scope > [data-owner="dsh-session-status"][data-owner-part="hover"]')
        if (!line) {
          line = document.createElement('div')
          line.setAttribute('data-owner', 'dsh-session-status')
          line.setAttribute('data-owner-part', 'hover')
          line.style.cssText = 'display:flex;align-items:center;gap:8px;font-size:12px;line-height:20px;color:#ADB2B8;'
          var icon = document.createElement('span')
          icon.style.cssText = 'display:inline-flex;align-items:center;flex:none;'
          line.appendChild(icon)
          line.appendChild(document.createElement('span'))
          container.appendChild(line)
        }
        line.setAttribute('data-session-id', id)
        line.setAttribute('data-status-key', label.key)
        updateIcon(line.children[0], label)
        if (line.children[1].textContent !== label.name) line.children[1].textContent = label.name
        kept.add(line)
      }
    }

    /** 统一对齐标记, 移除失效或已清除状态的节点, 保留未变化的节点. */
    /**
     * 推进「新会话默认进行中」: 判定结果记回本地进度, 再逐个补写 active。
     * 会话在写之前就已记进 seen, 所以 setSessionStatus 触发的重入渲染不会再判它。
     */
    function syncAutoActive(ctx) {
      var plan = planAutoActive({
        enabled: ctx.autoActive,
        armed: autoActiveRef.armed,
        seenIds: autoActiveRef.seen,
        ids: ctx.list.ids,
        summaries: ctx.list.byId,
        statuses: ctx.sessionsMap,
        statusesReady: statusRef.ready,
      })
      autoActiveRef.armed = plan.armed
      autoActiveRef.seen = plan.seenIds
      for (var i = 0; i < plan.targets.length; i++) setSessionStatus(plan.targets[i], 'active')
    }

    function renderAll() {
      var ctx = collectStatusContext()
      if (!ctx) { removeOwnNodes(); return }
      var kept = new Set()
      renderRowDots(ctx, kept)
      renderHoverStatus(ctx, kept)
      var nodes = document.querySelectorAll(ownNodeSelector())
      for (var i = 0; i < nodes.length; i++) {
        if (!kept.has(nodes[i])) nodes[i].remove()
      }
      syncAutoActive(ctx)
    }

    // ------------------------------------------------- 头部 pill + 选择器（1）

    // pill / 下拉 / 行状态点的样式都在 STYLE_CSS 里, 这里只负责 DOM 与 CSS 变量.

    /**
     * 标签象征 icon 的 SVG 标记（Material 风格 path，viewBox 24，颜色由标签色填充）。
     * 用 icon 而非圆点，避免与 DSH 内置会话运行状态点（黄/绿）视觉重复。
     * @param name - 'bolt' | 'check' | 'pause' | 'tag'。
     * @param color - 填充色（标签颜色）。
     * @param size - 边长像素。
     */
    function iconSvgMarkup(name, color, size) {
      var s = size || 12
      var inner
      switch (name) {
        case 'bolt': // 闪电：进行中
          inner = '<path d="M13 2 3 14h7l-1 8 10-12h-7l1-8z"/>'
          break
        case 'check': // 对勾：已结项
          inner = '<path d="M20 6 9 17l-5-5" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>'
          break
        case 'pause': // 暂停符：搁置中
          inner = '<path d="M6 5h4v14H6zM14 5h4v14h-4z"/>'
          break
        case 'flag':
          inner = '<path d="M14.4 6 14 4H5v17h2v-7h5.6l.4 2h7V6z"/>'
          break
        case 'star':
          inner = '<path d="M12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>'
          break
        case 'fire':
          inner = '<path d="M13.5.67s.74 2.65.74 4.8c0 2.06-1.35 3.73-3.41 3.73-2.07 0-3.63-1.67-3.63-3.73l.03-.36C5.21 7.51 4 10.62 4 14c0 4.42 3.58 8 8 8s8-3.58 8-8C20 8.61 17.41 3.8 13.5.67zM11.71 19c-1.78 0-3.22-1.4-3.22-3.14 0-1.62 1.05-2.76 2.81-3.12 1.77-.36 3.6-1.21 4.62-2.58.39 1.29.59 2.65.59 4.04 0 2.65-2.15 4.8-4.8 4.8z"/>'
          break
        case 'clock':
          inner = '<path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67z"/>'
          break
        case 'bookmark':
          inner = '<path d="M17 3H7c-1.1 0-2 .9-2 2v16l7-3 7 3V5c0-1.1-.9-2-2-2z"/>'
          break
        case 'rocket':
          inner = '<path d="M9.19 6.35c-2.04 2.29-3.44 5.58-3.57 5.89l-.55 1.18c-.02.06 0 .11.04.15l3.97 3.97c.04.04.09.06.15.04l1.18-.55c.31-.13 3.6-1.53 5.89-3.57L9.19 6.35zM20.66 3.34c-3.36-1.16-6.86.13-9.75 3.02-1.74 1.74-3.18 3.94-4.19 6.13-.24.51-.12 1.1.28 1.5l3.05 3.05c.4.4.99.52 1.5.28 2.19-1.01 4.39-2.45 6.13-4.19 2.89-2.89 4.18-6.39 3.02-9.75-.11-.32-.41-.54-.75-.54-.07 0-.18 0-.29.02zM6.59 15.41c-.35-.35-1.15-.09-1.5.25L3.3 18.45c-.4.4-.4 1.04 0 1.44l.81.81c.4.4 1.04.4 1.44 0l2.79-2.79c.34-.35.6-1.15.25-1.5z"/>'
          break
        case 'target':
          inner = '<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm0-14c-3.31 0-6 2.69-6 6s2.69 6 6 6 6-2.69 6-6-2.69-6-6-6zm0 9c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3z"/>'
          break
        case 'pin':
          inner = '<path d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"/>'
          break
        case 'bell':
          inner = '<path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.63-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.64 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/>'
          break
        default: // 标签形：自定义标签
          inner = '<path d="M21.41 11.58l-9-9C12.05 2.22 11.55 2 11 2H4c-1.1 0-2 .9-2 2v7c0 .55.22 1.05.59 1.42l9 9c.36.36.86.58 1.41.58.55 0 1.05-.22 1.41-.59l7-7c.37-.36.59-.86.59-1.41 0-.55-.23-1.06-.59-1.42zM5.5 7C4.67 7 4 6.33 4 5.5S4.67 4 5.5 4 7 4.67 7 5.5 6.33 7 5.5 7z"/>'
      }
      return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="' + s + '" height="' + s + '" fill="' + color + '" style="display:block">' + inner + '</svg>'
    }

    /** React 侧的 icon 包裹 span（flex none，供 pill/菜单/设置页复用）。 */
    function iconSpan(name, color, size) {
      return react.createElement('span', {
        style: { display: 'inline-flex', alignItems: 'center', flex: 'none' },
        dangerouslySetInnerHTML: { __html: iconSvgMarkup(name, color, size) },
      })
    }

    /** 菜单里「清除状态」那一项的 id (标签 key 都是小写字母开头, 不会撞). */
    var CLEAR_MENU_ITEM_ID = '__clear'

    /**
     * 头部状态 pill: 形状与右侧「在本地打开」同款 split 按钮, 下拉与悬浮提示
     * 交给原生 Menu / Tooltip —— 定位, 键盘走查, 外部点击关闭, 勾选标记都由宿主原子负责.
     */
    function StatusPill(props) {
      var sessionId = props && props.sessionId
      var tick = react.useReducer(function (x) { return x + 1 }, 0)
      var bump = tick[1]
      var openState = react.useState(false)
      var open = openState[0]
      var setOpen = openState[1]

      react.useEffect(function () {
        var offScope = scopeRef.scope ? scopeRef.scope.subscribe(bump) : null
        var offSessions = sessionsRef.list ? sessionsRef.list.subscribe(bump) : null
        var offStatus = subscribeStatus(bump)
        return function () {
          if (offScope) offScope()
          if (offSessions) offSessions()
          offStatus()
        }
      })

      var snap = scopeRef.scope ? scopeRef.scope.getSnapshot() : null
      var value = snap && snap.status === 'ready' && snap.value ? snap.value : null
      var labels = resolveLabels(value || {})
      var sessionsMap = statusMap()
      var currentKey = typeof sessionId === 'string' ? sessionsMap[sessionId] : undefined
      var current = currentKey ? findLabel(labels, currentKey) : undefined

      if (typeof sessionId !== 'string' || sessionId === '') return null

      // 标签色只用来淡染底色: 未设置状态时不写这两个变量, CSS 回落到中性 hover 色.
      var tint = current ? {
        '--dss-tint': rgbaFromHex(current.color, 0.1),
        '--dss-tint-strong': rgbaFromHex(current.color, 0.2),
      } : null

      var main = react.createElement(Tooltip, {
        key: 'main',
        portal: true,
        side: 'bottom',
        delayMs: 500,
        label: current ? cycleHint(current.name) : '设置会话状态 (单击循环: 进行中 → 已结项 → 搁置中 → 无)',
      }, react.createElement('button', {
        type: 'button',
        className: 'dss-pill-main',
        style: tint || undefined,
        'aria-label': current ? ('会话状态: ' + current.name + ', 单击循环切换') : '设置会话状态',
        onClick: function (event) {
          event.stopPropagation()
          setSessionStatus(sessionId, nextSessionStatus(currentKey))
        },
      }, current
        ? [iconSpan(resolveLabelIcon(current), current.color, 12), current.name]
        : [iconSpan('tag', 'currentColor', 12), '未设置状态']))

      var caret = react.createElement('button', {
        type: 'button',
        className: 'dss-pill-caret',
        'aria-label': '选择状态 (自定义标签 / 清除)',
        'aria-haspopup': 'menu',
        'aria-expanded': open ? 'true' : 'false',
        onClick: function (event) {
          event.stopPropagation()
          setOpen(!open)
        },
      }, react.createElement(IconChevronDownOutlineRegular, { size: 10 }))

      var items = []
      for (var i = 0; i < labels.length; i++) {
        items.push({
          id: labels[i].key,
          icon: iconSpan(resolveLabelIcon(labels[i]), labels[i].color, 12),
          label: labels[i].name,
        })
      }
      items.push({ type: 'separator', id: 'sep' })
      items.push({ id: CLEAR_MENU_ITEM_ID, icon: iconSpan('tag', 'currentColor', 12), label: '清除状态' })

      return react.createElement(Menu, {
        className: 'dss-pill-anchor',
        open: open,
        autoFocus: true,
        portal: true,
        dense: true,
        align: 'end',
        onClose: function () { setOpen(false) },
        onSelect: function (id) {
          setOpen(false)
          setSessionStatus(sessionId, id === CLEAR_MENU_ITEM_ID ? null : id)
        },
        items: items,
        selectedId: currentKey,
        anchor: react.createElement('div', {
          className: 'dss-pill',
          'data-unset': current ? 'false' : 'true',
        }, main, caret),
      })
    }

    // --------------------------------------------------- 设置页标签管理（3）

    function SettingsSection(props) {
      if (props.view === 'summary') {
        return '按会话维护状态标签, 在会话行与头部显示状态点, 并可让新会话默认进行中.'
      }
      var tick = react.useReducer(function (x) { return x + 1 }, 0)
      var bump = tick[1]
      var nameDraft = react.useState('')
      var draft = nameDraft[0]
      var setDraft = nameDraft[1]
      var colorState = react.useState(PALETTE[0])
      var colorPick = colorState[0]
      var setColorPick = colorState[1]
      var iconState = react.useState('tag')
      var iconPick = iconState[0]
      var setIconPick = iconState[1]
      // 「新会话默认进行中」开关的写入状态: 与宿主设置行一样, 写入中禁用, 失败报错.
      var busyState = react.useState(false)
      var toggleBusy = busyState[0]
      var setToggleBusy = busyState[1]
      var failState = react.useState(false)
      var toggleFailed = failState[0]
      var setToggleFailed = failState[1]

      react.useEffect(function () {
        var off = scopeRef.scope ? scopeRef.scope.subscribe(bump) : null
        return function () { if (off) off() }
      }, [])

      var snap = scopeRef.scope ? scopeRef.scope.getSnapshot() : null
      var value = snap && snap.status === 'ready' && snap.value ? snap.value : null
      var labels = resolveLabels(value || {})
      var builtins = labels.filter(function (label) { return label.builtin })
      var customs = labels.filter(function (label) { return !label.builtin })
      var overrides = (value && value.overrides) || {}
      var autoActive = !!(value && value.defaultActive === true)

      function isBuiltinOverridden(key) {
        var def = builtinDefaultColor(key)
        var ov = overrides[key]
        return !!(ov && ov.color && ov.color !== def)
      }

      function existingKeys() {
        var set = new Set()
        for (var i = 0; i < labels.length; i++) set.add(labels[i].key)
        return set
      }

      function commitCustomLabel(label) {
        var next = customs.map(function (l) { return l.key === label.key ? label : l })
        writeLabels(next)
      }

      function removeCustomLabel(key) {
        writeLabels(customs.filter(function (label) { return label.key !== key }))
      }

      function addLabel() {
        var name = draft.trim()
        if (name === '') return
        var key = generateLabelKey(name, existingKeys())
        writeLabels(customs.concat([{ key: key, name: name, color: colorPick, icon: iconPick }]))
        setDraft('')
        setColorPick(nextPaletteColor(labels))
        setIconPick('tag')
      }

      function swatchRow(onPick, current) {
        return PALETTE.map(function (color) {
          var active = color === current
          return react.createElement('button', {
            key: color,
            type: 'button',
            title: color,
            style: {
              width: '16px',
              height: '16px',
              borderRadius: '50%',
              border: active ? '2px solid var(--dsw-alias-label-primary, #fff)' : '1px solid rgba(128,128,128,0.4)',
              background: color,
              cursor: 'pointer',
              padding: '0',
            },
            onClick: function () { onPick(color) },
          })
        })
      }

      /** 自定义标签 icon 选择器（LABEL_ICONS 全部可选，选中项高亮）。 */
      function iconRow(onPick, current, size) {
        var s = size || 13
        return react.createElement('div', { style: { display: 'flex', gap: '3px', flexWrap: 'wrap' } },
          LABEL_ICONS.map(function (iconName) {
            var active = iconName === current
            return react.createElement('button', {
              key: iconName,
              type: 'button',
              title: iconName,
              style: {
                display: 'inline-flex',
                alignItems: 'center',
                justifyContent: 'center',
                width: s + 10,
                height: s + 10,
                border: active
                  ? '1.5px solid var(--dsw-alias-label-primary, #fff)'
                  : '1px solid rgba(128,128,128,0.35)',
                borderRadius: '6px',
                background: 'transparent',
                cursor: 'pointer',
                padding: '2px',
              },
              dangerouslySetInnerHTML: {
                __html: iconSvgMarkup(iconName, active ? '#ddd' : '#999', s),
              },
              onClick: function () { onPick(iconName) },
            })
          }))
      }

      /**
       * 任意十六进制颜色输入：失焦/回车提交（规范化后），非法输入回退当前色。
       * 外部颜色变化（点色板）时同步草稿。
       */
      function ColorHexInput(props) {
        var draftState = react.useState(props.color)
        var draft = draftState[0]
        var setDraft = draftState[1]
        react.useEffect(function () { setDraft(props.color) }, [props.color])
        function commit() {
          var hex = normalizeHex(draft)
          if (hex) props.onCommit(hex)
          else setDraft(props.color)
        }
        return react.createElement(Input, {
          style: { width: '78px', fontFamily: 'monospace' },
          value: draft,
          title: '任意颜色 #RRGGBB（或 #RGB 简写）',
          'aria-label': '自定义颜色值',
          onChange: function (event) { setDraft(event.target.value) },
          onBlur: commit,
          onKeyDown: function (event) { if (event.key === 'Enter') event.currentTarget.blur() },
        })
      }

      var headerStyle = { fontSize: '14px', fontWeight: 600, color: 'var(--dsw-alias-label-primary, #ddd)', margin: '0 0 10px' }
      var subStyle = { fontSize: '12px', color: 'var(--dsw-alias-label-tertiary, #888)', margin: '14px 0 6px' }
      var rowStyle = { display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 0' }
      var hintStyle = { fontSize: '11px', lineHeight: '16px', color: 'var(--dsw-alias-label-tertiary, #777)' }

      var builtinRows = builtins.map(function (label) {
        return react.createElement('div', { key: label.key, style: { padding: '2px 0' } },
          react.createElement('div', { style: rowStyle },
            iconSpan(resolveLabelIcon(label), label.color, 12),
            react.createElement('span', { style: { flex: '1' } }, label.name),
            react.createElement('span', { style: hintStyle }, '内置'),
            isBuiltinOverridden(label.key)
              ? react.createElement(Button, {
                size: 'sm',
                variant: 'ghost',
                onClick: function () { writeBuiltinOverride(label.key, builtinDefaultColor(label.key)) },
              }, '恢复默认')
              : null),
          react.createElement('div', { style: Object.assign({}, rowStyle, { marginLeft: '20px' }) },
            swatchRow(function (color) { writeBuiltinOverride(label.key, color) }, label.color)))
      })

      var customRows = customs.map(function (label) {
        return react.createElement('div', { key: label.key, style: { padding: '2px 0' } },
          react.createElement('div', { style: rowStyle },
            iconSpan(resolveLabelIcon(label), label.color, 12),
            react.createElement(Input, {
              className: 'dss-grow',
              value: label.name,
              'aria-label': '标签名称',
              onChange: function (event) {
                commitCustomLabel({ key: label.key, name: event.target.value, color: label.color, icon: label.icon })
              },
              onBlur: function () {
                if (label.name.trim() === '') commitCustomLabel({ key: label.key, name: '未命名', color: label.color, icon: label.icon })
              },
            }),
            react.createElement('div', { style: { display: 'flex', gap: '4px', alignItems: 'center' } },
              swatchRow(function (color) { commitCustomLabel({ key: label.key, name: label.name, color: color, icon: label.icon }) }, label.color),
              react.createElement(ColorHexInput, {
                color: label.color,
                onCommit: function (color) { commitCustomLabel({ key: label.key, name: label.name, color: color, icon: label.icon }) },
              })),
            react.createElement(Button, {
              size: 'sm',
              variant: 'ghost',
              className: 'dss-danger',
              onClick: function () { removeCustomLabel(label.key) },
            }, '删除')),
          react.createElement('div', { style: Object.assign({}, rowStyle, { marginLeft: '20px' }) },
            iconRow(function (iconName) { commitCustomLabel({ key: label.key, name: label.name, color: label.color, icon: iconName }) }, resolveLabelIcon(label))))
      })

      return react.createElement('div', { style: { padding: '4px 2px' } },
        react.createElement('div', { style: headerStyle }, '会话状态标签'),
        // 「新会话默认进行中」: 照宿主设置行写 (左边标题 + 说明, 右边原生 Switch;
        // 写入中禁用, 失败时 role="alert" 报错, 与「通用设置 → 开发者工具」同一套写法).
        react.createElement('div', {
          style: {
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'space-between',
            gap: '24px',
            padding: '16px 0',
            borderBottom: '0.5px solid var(--dsw-alias-border-l2, rgba(128,128,128,.25))',
          },
        },
          react.createElement('div', { style: { minWidth: '0' } },
            react.createElement('div', { style: { fontSize: '14px', lineHeight: '20px', color: 'var(--dsw-alias-label-primary, #ddd)' } }, '新会话默认进行中'),
            react.createElement('div', { style: { marginTop: '4px', fontSize: '12px', lineHeight: '18px', color: 'var(--dsw-alias-label-secondary, #aaa)' } },
              '打开后, 会话第一次出现在列表里时自动记一条「进行中」; 已有会话不受影响, 手动清除后也不会补回.'),
            toggleFailed ? react.createElement('div', {
              role: 'alert',
              style: { marginTop: '4px', fontSize: '12px', lineHeight: '18px', color: 'var(--dsw-alias-state-error-primary, #f87171)' },
            }, '写入被宿主拒绝; 如果刚更新过插件, 需要重启 dsh web 让宿主重新注册配置字段.') : null),
          react.createElement(Switch, {
            checked: autoActive,
            disabled: toggleBusy,
            label: '新会话默认进行中',
            onChange: function (next) {
              setToggleFailed(false)
              setToggleBusy(true)
              var pending = writeDefaultActive(next)
              if (!pending || typeof pending.then !== 'function') { setToggleBusy(false); return }
              pending.then(function (ok) {
                setToggleFailed(ok === false)
                setToggleBusy(false)
              }, function () {
                setToggleFailed(true)
                setToggleBusy(false)
              })
            },
          })),
        react.createElement('div', { style: subStyle }, '内置标签（名称固定，颜色可调）'),
        builtinRows,
        react.createElement('div', { style: subStyle }, '自定义标签'),
        customRows.length === 0
          ? react.createElement('div', { style: Object.assign({}, hintStyle, { padding: '4px 0' }) }, '暂无自定义标签')
          : customRows,
        react.createElement('div', { style: Object.assign({}, rowStyle, { marginTop: '10px', flexWrap: 'wrap' }) },
          react.createElement(Input, {
            className: 'dss-grow',
            placeholder: '新标签名称…',
            value: draft,
            'aria-label': '新标签名称',
            onChange: function (event) { setDraft(event.target.value) },
            onKeyDown: function (event) { if (event.key === 'Enter') addLabel() },
          }),
          react.createElement('div', { style: { display: 'flex', gap: '4px', alignItems: 'center' } },
            swatchRow(setColorPick, colorPick),
            react.createElement(ColorHexInput, { color: colorPick, onCommit: setColorPick })),
          react.createElement(Button, { size: 'sm', variant: 'primary', onClick: addLabel }, '添加')),
        react.createElement('div', { style: Object.assign({}, rowStyle, { marginTop: '4px' }) },
          react.createElement('span', { style: hintStyle }, 'icon'),
          iconRow(setIconPick, iconPick))
      )
    }

    // ---------------------------------------------------------------- apply

    var inject = ['slots', 'configForms', 'sessions']

    function apply(ctx) {
      var scope = ctx.configForms.get('dsh-session-status')
      scopeRef.scope = scope
      sessionsRef.list = ctx.sessions.list

      var disposers = []
      ensureStyleSheet()
      disposers.push(removeStyleSheet)
      disposers.push(scope.subscribe(renderAll))
      disposers.push(ctx.sessions.list.subscribe(renderAll))
      disposers.push(subscribeStatus(renderAll))
      disposers.push(installRowDots())

      // 首次拉取 + 低频轮询; 窗口重新聚焦/标签页可见时立即补一次, 覆盖其它
      // 标签页或浏览器改过状态的情况。
      void refreshStatuses()
      var timer = setInterval(function () {
        if (document.visibilityState !== 'hidden') void refreshStatuses()
      }, STATUS_POLL_MS)
      function onWindowActive() { void refreshStatuses() }
      window.addEventListener('focus', onWindowActive)
      document.addEventListener('visibilitychange', onWindowActive)
      disposers.push(function () {
        clearInterval(timer)
        window.removeEventListener('focus', onWindowActive)
        document.removeEventListener('visibilitychange', onWindowActive)
      })

      ctx.effect(function () {
        return function () {
          for (var i = 0; i < disposers.length; i++) disposers[i]()
        }
      }, 'dsh-session-status: row dots')

      ctx.slots.inject('conversation.session.header.utilities', function () {
        return ctx.slots.register({
          name: 'conversation.session.header.utilities',
          id: 'dsh-session-status',
          // 排在 DSH 自带「在本地打开」(order -10) 左边, 成为该排最左条目.
          order: -20,
        }, function (props) { return react.createElement(StatusPill, props) })
      })

      ctx.effect(function () {
        return ctx.configForms.whileServed(['dsh-session-status'], function () {
          return ctx.slots.inject('plugins.bundle.config', function () {
            return ctx.slots.register({
              name: 'plugins.bundle.config',
              key: 'dsh-session-status',
            }, function (props) { return react.createElement(SettingsSection, props) })
          })
        })
      }, 'dsh-session-status: plugins page card')
    }

    exports.inject = inject
    exports.apply = apply
    exports.__statusUtils = __statusUtils
    return module.exports
  },
})
