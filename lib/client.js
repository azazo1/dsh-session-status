...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function collectStatusContext() {
      ...
      var list = sessionsRef.list ? sessionsRef.list.getSnapshot() : null
      if (!list || list.phase !== 'ready') return null
      return { list: list, labels: resolveLabels(snap.value), sessionsMap: statusMap() }
    }
    ...
    function renderRowDots(ctx, kept) {
      ...
      for (var r = 0; r < rows.length; r++) {
        ...
        if (mark.title !== label.name) mark.title = label.name
    ...
    function StatusPill(props) {
      ...
      var pill = react.createElement('button', {
        type: 'button',
        style: pillStyle,
        title: current ? (current.name + '（点击循环：进行中 → 已结项 → 搁置中 → 无）') : '设置会话状态（点击循环：进行中 → 已结项 → 搁置中 → 无）',
    ...
    function SettingsSection(props) {
      if (props.view === 'summary') {
        return '按会话维护状态标签, 并在会话行与头部显示状态点.'
      }
      var tick = react.useReducer(function (x) { return x + 1 }, 0)
      var bump = tick[1]
