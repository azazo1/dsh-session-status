 ...
 *
 * 装配两件事:
 *  1. plugin Config `dsh-session-status`: 只留标签定义 (labels) 与内置三态
 *     颜色覆盖 (overrides) 这类用户可编辑的偏好; 内置三态由浏览器固定合并.
...
/**
 * settings namespace 的 v1 结构: 自定义标签列表 + 内置标签颜色覆盖 + 新会话默认态。
...
export const Config = z.object({
  labels: z.array(LabelSchema).default([]).volatile(),
  // 内置三态的颜色覆盖（key → { color }），用户层写入；不写则用内置默认色。
  overrides: z.dict(z.object({ color: z.string() })).default({}).volatile(),
  // 「新会话默认进行中」开关: 打开后由浏览器侧在会话首次出现时补一条 active
  // 记录 (见 lib/status-store.js 的 planAutoActive), host 只负责存这个偏好。
  defaultActive: z.boolean().default(false).volatile(),
})

/** @param ctx - 宿主上下文（inject: ['settings'] 保证 ctx.settings 可用）。 */
export function apply(ctx) {
  // domain 与 HTTP 路由是 web 侧才有的服务; 缺失时这一半不装配.
