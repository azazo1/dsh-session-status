...
/**
 * 把标签色换算成 rgba(r, g, b, alpha); 非法颜色返回 null, alpha 越界时夹到 [0, 1].
 * 浏览器端用它在不依赖 color-mix() 的前提下现算淡染底色与描边.
 */
export function rgbaFromHex(value, alpha) {
  const hex = normalizeHex(value)
  if (hex === null) return null
  const r = Number.parseInt(hex.slice(1, 3), 16)
  const g = Number.parseInt(hex.slice(3, 5), 16)
  const b = Number.parseInt(hex.slice(5, 7), 16)
  const raw = typeof alpha === 'number' && Number.isFinite(alpha) ? alpha : 1
...
/**
 * 会话状态记录 key 的字符集：storage 的 per-record 布局把记录 key 当文件路径段,
 * 只接受 [A-Za-z0-9_-]。session id 形如 `session-<uuid>`, 天然满足。
 */
export const RECORD_KEY_PATTERN = /^[a-zA-Z0-9_-]+$/

/** session id 是否可作为 storage 记录 key（即 per-record 布局的路径段）。 */
export function isRecordKeySafe(sessionId) {
  return typeof sessionId === 'string' && RECORD_KEY_PATTERN.test(sessionId)
}
 ...
 * 浏览器侧用它做乐观更新（先改本地快照再发请求），host 侧的权威写入走
...
/**
 * 「新会话默认进行中」的判定：从当前会话列表里挑出需要补写 active 的会话。
 *
 * 语义：只在客户端亲眼看到某个会话第一次出现时补一条「进行中」记录, 不回头改
 * 已有会话, 也不覆盖任何已有状态。因此:
 *  - `armed === false` 的这一轮只取基线（当前已有会话一律不动）, 不写数据;
 *  - 占位空白会话不算「已经出现」, 等它有内容后再判;
 *  - 状态快照未到位（`statusesReady !== true`）时不写, 否则可能覆盖已有状态;
 *  - 已经判过的会话即使被手动清除, 也不会被自动补回。
 *
 * @param input - { enabled, armed, seenIds, ids, summaries, statuses, statusesReady }。
 * @returns {{ armed: boolean, seenIds: string[], targets: string[] }}
 *   新的基线标记, 已判过的会话 id, 以及需要写 active 的会话 id。
 */
export function planAutoActive(input) {
  const seen = new Set(input.seenIds || [])
  if (input.enabled !== true) return { armed: false, seenIds: [], targets: [] }
  const ids = Array.isArray(input.ids) ? input.ids : []
  const summaries = input.summaries || {}
  const statuses = input.statuses || {}
  const arming = input.armed !== true
  const targets = []
  for (const id of ids) {
    if (!arming && seen.has(id)) continue
    const summary = summaries[id]
    // 身份未确认或还没有内容的占位会话留到下一轮：它长出内容时才算新会话。
  ...
  return { armed: true, seenIds: Array.from(seen), targets }
}
